...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navigation = [
    ...
    { name: 'Diagnostics', href: '/diagnostics', icon: FileText, permission: 'VIEW_ANALYTICS' },
    { 
      name: 'Settings', 
      href: '/settings', 
      icon: Settings,
    ...
    { name: 'Audit Logs', href: '/audit-logs', icon: FileText, permission: 'VIEW_ANALYTICS' },
    { name: 'Support', href: '/support', icon: LifeBuoy, permission: 'MANAGE_USERS' },
...
function AdminNotifications() {
  ...
  React.useEffect(() => {
    ...
    const fetchNotes = async () => {
      try {
        setError(null);
        const notes = await (await import('../services/apiService')).apiService.getAdminNotifications();
        if (isMounted) setItems(Array.isArray(notes) ? notes : []);
      } catch (e) {
        if (!isMounted) return;
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="p-2 rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100">🔔 {unread || ''}</button>
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded shadow-lg max-h-96 overflow-auto z-50">
          {error && <div className="px-3 py-2 text-sm text-red-500 border-b">{error}</div>}
          {items.map(n => (
            <a key={n.id} href={resolveLink(n)} className="block px-3 py-2 text-sm border-b hover:bg-gray-50">
              <div className="font-medium text-gray-800">{n.type}</div>
              <div className="text-gray-600">{(n.message || '').slice(0,120)}</div>
              <div className="text-xs text-gray-400">{new Date(n.createdAt).toLocaleString()}</div>
            </a>
          ))}
          {!error && items.length === 0 && <div className="px-3 py-4 text-sm text-gray-500">No notifications</div>}
        </div>
      )}
    </div>
  );
...
function resolveLink(n: any) {
  const t = (n.type || '').toString();
  if (t.includes('deposit')) return '/payments';
  if (t.includes('withdraw')) return '/payments';
  if (t.includes('round')) return '/rounds';
  if (t.includes('bet')) return '/bets';
  if (t.includes('feedback')) return '/feedbacks';
  if (t.includes('support')) return '/support';
  if (t.includes('chat')) return '/users';
  if (t.includes('register')) return '/users';
  return '/dashboard';
}