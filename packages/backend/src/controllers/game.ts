...
router.get('/current-round', optionalAuth, asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  bets.forEach(bet => {
    ...
    switch (bet.betType) {
      ...
      case 'ODD_EVEN':
        if (betValue === 'odd') {
          distribution.oddEven.odd.count++;
          distribution.oddEven.odd.amount += bet.amount;
        } else if (betValue === 'even') {
          distribution.oddEven.even.count++;
          distribution.oddEven.even.amount += bet.amount;
        }
        break;
    }
  });
...
// Calculate payout endpoint
router.post('/calculate-payout', optionalAuth, asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { betType, bets, winningNumber } = req.body;

  // Validate input
  if (!betType || !bets || !Array.isArray(bets) || winningNumber === undefined) {
    throw new ValidationError('Missing required fields: betType, bets, winningNumber');
  }

  if (winningNumber < 0 || winningNumber > 9) {
    throw new ValidationError('Winning number must be between 0 and 9');
  }

  // Calculate payout using the utility function
  const payout = calculateBetsPayout(bets, winningNumber);
...
// Get total game history (public) - simplified for history page
router.get('/history/total', optionalAuth, asyncHandler(async (req: AuthenticatedRequest, res) => {
  const query = paginationSchema.parse(req.query);
  const { page, pageSize } = query;
...
router.post('/bet', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res) => {
  ...
  try {
    // Expand the bet to show what will be placed
    ...
    res.status(201).json(createSuccessResponse({
      success: true,
      totalBet: betExpansion.totalBet,
      bets: betExpansion.bets,
      betType: betExpansion.betType,
...
// Get winning number for current round (during spin preparation phase)
router.get('/winning-number', optionalAuth, asyncHandler(async (req: AuthenticatedRequest, res) => {
  if (!gameEngine) {
    throw new GameError('Game engine not available');
  }
  ...
  // Only allow fetching winning number during spin preparation or spinning phases
  const currentPhase = gameEngine.getCurrentPhase();
  if (currentPhase !== 'SPIN_PREPARATION' && currentPhase !== 'SPINNING') {
    throw new GameError('Winning number not available in current phase');
  }

  // Get the winning number from the phase manager
  const winningNumber = gameEngine.getWinningNumber();
  if (winningNumber === null) {
    throw new GameError('Winning number not calculated yet');
  }

  // Get bet distribution for verification
  const betDistribution = gameEngine.getBetDistribution();
