...
export const paginationSchema = z.object({
  ...
  pageSize: z.string().transform((val) => parseInt(val, 10)).pipe(z.number().int().min(1).max(100)).default('20'),
});
...
export const transactionHistorySchema = paginationSchema.extend({
  userId: z.string().uuid().optional(),
  type: z.enum(['deposit', 'withdrawal', 'bet_placed', 'bet_won', 'bet_lost', 'cashback', 'DEPOSIT', 'WITHDRAWAL', 'BET_PLACED', 'BET_WON', 'BET_LOST', 'CASHBACK']).optional(),
  status: z.enum(['pending', 'approved', 'rejected', 'completed', 'cancelled', 'PENDING', 'APPROVED', 'REJECTED', 'COMPLETED', 'CANCELLED']).optional(),
  wallet: z.enum(['BETTING', 'GAMING']).optional(),
  ...dateRangeSchema.shape,
});
